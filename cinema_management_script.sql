...
(
    ...
    constraint cinema_room_ibfk_1
        foreign key (cinema_id) references cinema (id),
    constraint cinema_room_ibfk_2
...
create index cinema_layout_id
    on cinema_room (cinema_layout_id);
...
create table m2m_cinema_layout
(
    cinema_id        binary(16) not null,
    cinema_layout_id binary(16) not null,
    constraint FKafapjkmt7fxwfu6sd839wv65t
        foreign key (cinema_id) references cinema (id),
    constraint FKg80c3r1r3crpitew2ckvtajt
        foreign key (cinema_layout_id) references cinema_layout (id)
);
...
(
    ...
    constraint FKeu7wyv7o56777oo642mwij238
        foreign key (cinema_room_id) references cinema_room (id),
    constraint perform_ibfk_1
        foreign key (film_id) references film (id),
    constraint perform_ibfk_4
...
(
    ...
    constraint UK_anw8yba21b9lbv92gdbuap1v3
        unique (payments_id),
    constraint FKlbtxyk9mdohidwx6ogmgtvqb5
        foreign key (item_id) references item (id),
    constraint FKqvdmu6jgcm8khkodg4c4a275m
...
create index perform_id
    on payment (perform_id);

create index user_id
    on payment (user_id);

create index dest_id
    on perform (dest_id);
...
(
    ...
    constraint FKt7a8nyoomn5js0uye7a4s3ste
...
(
    ...
    constraint FKal69sspaj8nmlaq5288o63klr
        foreign key (perform_id) references perform (id)
);
...
create table m2m_film_tag
(
    film_id binary(16) not null,
    tag_id  binary(16) not null,
    constraint FKeenhf0isewcem0g3k3p4gc4b1
        foreign key (film_id) references film (id),
    constraint FKjxxi3qbe4vliyixnksmx5151k
...
(
    ...
    constraint cinema_manager_ibfk_1
        foreign key (cinema_id) references cinema (id),
    constraint cinema_manager_ibfk_2
        foreign key (user_id) references users (id)
);

create index user_id
    on cinema_manager (user_id);
...
(
    ...
    constraint FKcdfhjbqb1ypqiu9kdaitfok1s
        foreign key (perform_id) references perform (id),
    constraint FKde2kvigd2bvo711m053w73i65
        foreign key (user_id) references users (id)
);
